...
const I18N = {
  en: {
    ...
    car_trip_type: "Trip type", car_open_map: "Open map", car_add_favorite: "Add favorite", car_favorite_saved: "Favorite route saved ✓",
    car_favorites: "Favorite routes", car_no_favorites: "No favorite routes yet.", car_extra_toll: "Toll / bridge", car_extra_parking: "Parking / ferry", car_extra_other: "Other", car_extra_total: "Extra costs", car_total_trip: "Trip total",
  ...
  tr: {
    ...
    car_trip_type: "Yolculuk tipi", car_open_map: "Haritada aç", car_add_favorite: "Favoriye ekle", car_favorite_saved: "Favori rota kaydedildi ✓",
    car_favorites: "Favori rotalar", car_no_favorites: "Henüz favori rota yok.", car_extra_toll: "Otoyol / köprü", car_extra_parking: "Otopark / feribot", car_extra_other: "Diğer", car_extra_total: "Ek giderler", car_total_trip: "Yolculuk toplamı",
...
const state = {
  ...
  vehicleHub: {
    activeVehicle: "", trips: [], favorites: [],
    fromP: "", fromD: "", toP: "", toD: "", lastRoute: null, seq: 0, favSeq: 0, tripMode: "round",
    extras: { toll: 0, parking: 0, other: 0 },
  },
  portfolio: {
    holdings: [], seq: 0,
...
const el = {
  ...
  carTripOptions: document.getElementById("carTripOptions"),
  carTripMode: document.getElementById("carTripMode"),
  carExtraToll: document.getElementById("carExtraToll"),
  carExtraParking: document.getElementById("carExtraParking"),
  carExtraOther: document.getElementById("carExtraOther"),
  carMapLink: document.getElementById("carMapLink"),
  carFavAdd: document.getElementById("carFavAdd"),
  carFavWrap: document.getElementById("carFavWrap"),
  carFavList: document.getElementById("carFavList"),
...
// ============================================================
// Türkiye route location data is loaded from data/tr-locations.js.
...
function routeMapUrl(r) {
  if (!r) return "#";
  return `https://www.google.com/maps/dir/?api=1&origin=${encodeURIComponent(r.from)}&destination=${encodeURIComponent(r.to)}&travelmode=driving`;
}
function tripExtrasTotal(extras) {
  const e = extras || {};
  return (e.toll || 0) + (e.parking || 0) + (e.other || 0);
}
function selectedTripFuel(costOne, mode) {
  if (costOne == null) return null;
  return mode === "one" ? costOne : costOne * 2;
}
function currentTripExtras() {
  return {
    toll: parseNumber(el.carExtraToll ? el.carExtraToll.value : "0"),
    parking: parseNumber(el.carExtraParking ? el.carExtraParking.value : "0"),
    other: parseNumber(el.carExtraOther ? el.carExtraOther.value : "0"),
  };
}
function updateTripOptionSummary() {
  const h = state.vehicleHub, r = h.lastRoute;
  if (!r || !el.carResults) return;
  h.tripMode = el.carTripMode ? el.carTripMode.value : (h.tripMode || "round");
  h.extras = currentTripExtras();
  const costOne = fuelCost(r.km, activeVehicle());
  const fuel = selectedTripFuel(costOne, h.tripMode);
  const extras = tripExtrasTotal(h.extras);
  const total = (fuel || 0) + extras;
  const fuelLabel = h.tripMode === "one" ? t("car_cost_one") : t("car_cost_round");
  const fuelNode = el.carResults.querySelector("[data-trip-fuel]");
  const extraNode = el.carResults.querySelector("[data-trip-extra]");
  const totalNode = el.carResults.querySelector("[data-trip-total]");
  if (fuelNode) fuelNode.textContent = `${fuelLabel}: ${fuel == null ? "—" : formatMoney(fuel)}`;
  if (extraNode) extraNode.textContent = `${t("car_extra_total")}: ${formatMoney(extras)}`;
...
function renderCarRoute() {
  ...
  renderCarFavorites();
  if (!r) {
    el.carResults.hidden = true;
    el.carTripOptions.hidden = true;
    el.carRouteMsg.hidden = true;
    return;
  }
  el.carResults.hidden = false;
  el.carTripOptions.hidden = false;
  const veh = activeVehicle();
  const costOne = fuelCost(r.km, veh), costRound = costOne == null ? null : costOne * 2;
  const extras = tripExtrasTotal(h.extras);
  const selectedFuel = selectedTripFuel(costOne, h.tripMode || "round");
  const selectedTotal = (selectedFuel || 0) + extras;
  ...
  el.carResults.innerHTML = `
    ...
    <div class="car-trip-total">
      <div><span data-trip-fuel>${(h.tripMode === "one" ? t("car_cost_one") : t("car_cost_round"))}: ${selectedFuel == null ? "—" : formatMoney(selectedFuel)}</span><span data-trip-extra>${t("car_extra_total")}: ${formatMoney(extras)}</span></div>
      <strong data-trip-total>${formatMoney(selectedTotal)}</strong>
    </div>
    ${costOne == null ? `<p class="car-hint">${t("car_no_profile")}</p>` : ""}`;
  if (el.carTripMode) el.carTripMode.value = h.tripMode || "round";
  if (el.carExtraToll) el.carExtraToll.value = h.extras && h.extras.toll ? formatThousands(h.extras.toll) : "";
  if (el.carExtraParking) el.carExtraParking.value = h.extras && h.extras.parking ? formatThousands(h.extras.parking) : "";
  if (el.carExtraOther) el.carExtraOther.value = h.extras && h.extras.other ? formatThousands(h.extras.other) : "";
  if (el.carMapLink) el.carMapLink.href = routeMapUrl(r);
  el.carRouteMsg.hidden = true;
}
...
function saveCarTrip() {
  const h = state.vehicleHub, r = h.lastRoute;
  if (!r) return;
  updateTripOptionSummary();
  const veh = activeVehicle();
  const costOne = fuelCost(r.km, veh);
  const mode = h.tripMode || "round";
  const fuelSelected = selectedTripFuel(costOne, mode);
  const extras = { ...(h.extras || {}) };
  const extraTotal = tripExtrasTotal(extras);
  h.trips.unshift({
    id: "ct" + ++h.seq, date: new Date().toISOString().slice(0, 10),
    from: r.from, to: r.to, km: r.km, mins: r.mins, mode,
    fuelOne: costOne, fuelRound: costOne == null ? null : costOne * 2, fuelSelected,
    extras, extraTotal, total: (fuelSelected || 0) + extraTotal,
...
function addFavoriteRoute() {
  const h = state.vehicleHub, r = h.lastRoute;
  if (!r) return;
  const fav = { id: "cf" + ++h.favSeq, fromP: h.fromP, fromD: h.fromD, toP: h.toP, toD: h.toD, label: `${r.from} → ${r.to}` };
  h.favorites = (h.favorites || []).filter((x) => !(x.fromP === fav.fromP && x.fromD === fav.fromD && x.toP === fav.toP && x.toD === fav.toD));
  h.favorites.unshift(fav);
  renderCarFavorites();
  toastCar(t("car_favorite_saved"));
  saveState();
}
function applyFavoriteRoute(fav) {
  const h = state.vehicleHub;
  h.fromP = fav.fromP || ""; h.fromD = fav.fromD || ""; h.toP = fav.toP || ""; h.toD = fav.toD || "";
  fillProvinceSelect(el.carFromP, h.fromP); fillProvinceSelect(el.carToP, h.toP);
  fillDistrictSelect(el.carFromD, h.fromP, h.fromD); fillDistrictSelect(el.carToD, h.toP, h.toD);
  clearCarRouteResult();
  saveState();
}
function renderCarFavorites() {
  if (!el.carFavWrap || !el.carFavList) return;
  const h = state.vehicleHub;
  const favs = h.favorites || [];
  el.carFavWrap.hidden = !favs.length;
  if (!favs.length) { el.carFavList.innerHTML = ""; return; }
  el.carFavList.innerHTML = favs.map((f) => `<div class="car-fav"><button type="button" data-fav-use="${f.id}">${escapeHtml(f.label)}</button><button class="cat-remove" type="button" data-fav-del="${f.id}" aria-label="remove">×</button></div>`).join("");
  el.carFavList.querySelectorAll("[data-fav-use]").forEach((b) => b.addEventListener("click", () => {
    const fav = favs.find((x) => x.id === b.dataset.favUse);
    if (fav) applyFavoriteRoute(fav);
  }));
  el.carFavList.querySelectorAll("[data-fav-del]").forEach((b) => b.addEventListener("click", () => {
    h.favorites = (h.favorites || []).filter((x) => x.id !== b.dataset.favDel);
...
function buildCarHistory() {
  ...
  h.trips.forEach((tr) => {
    ...
    const mode = tr.mode || "round";
    const total = tr.total != null ? tr.total : (mode === "one" ? tr.fuelOne : tr.fuelRound);
    row.innerHTML = `
      <div class="car-trip-main">
        <div class="car-trip-route"><b>${tr.from} → ${tr.to}</b><span class="car-trip-date">${tr.date}</span></div>
        <div class="car-trip-meta">${fmtKm(tr.km)} · ${fmtDuration(tr.mins)} · ${mode === "one" ? t("car_oneway_label") : t("car_roundtrip_label")}${tr.extraTotal ? ` · ${t("car_extra_total")} ${formatMoney(tr.extraTotal)}` : ""}</div>
      </div>
      <div class="car-trip-right">
        <span class="car-trip-cost">${total == null ? "—" : formatMoney(total)}</span><small>${t("car_total_trip")}</small>
  ...
  const total = h.trips.reduce((a, tr) => a + (tr.total != null ? tr.total : (tr.fuelRound || 0)), 0);
  if (total > 0) {
...
el.carFavAdd.addEventListener("click", addFavoriteRoute);
el.carTripMode.addEventListener("change", () => { state.vehicleHub.tripMode = el.carTripMode.value; updateTripOptionSummary(); saveState(); });
[el.carExtraToll, el.carExtraParking, el.carExtraOther].forEach((input) => input.addEventListener("input", () => { state.vehicleHub.extras = currentTripExtras(); updateTripOptionSummary(); saveState(); }));
...
function loadState() {
  ...
  if (s.vehicleHub && typeof s.vehicleHub === "object") {
    ...
    state.vehicleHub = {
      ...
      lastRoute: v.lastRoute || null, seq: v.seq || 0, favSeq: v.favSeq || (Array.isArray(v.favorites) ? v.favorites.length : 0),
      tripMode: v.tripMode || "round", extras: v.extras || { toll: 0, parking: 0, other: 0 },
    };
